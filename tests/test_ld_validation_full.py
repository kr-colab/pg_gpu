...
class TestLDValidationFull:
    ...
    def gpu_results(self, im_model_data, moments_results):
        """Compute GPU results on the is_biallelic_01 subset.

        pg_gpu's single LD mode keeps two-present {0,2}/{1,2} sites moments.LD
        drops, so parity holds only on the {0,1} subset; moments filters there
        internally, so restrict the GPU input to match.
        """
        vcf_path, pop_file = im_model_data
        ...
        bi01 = allel.GenotypeArray(
            vcf['calldata/GT']).count_alleles().is_biallelic_01()
        h_gpu = HaplotypeMatrix.from_vcf(vcf_path).get_subset(np.where(bi01)[0])
    ...
    def test_fixture_contains_multiallelic_sites(self, im_model_data):
        """Pin: the fixture has non-{0,1} sites, so the {0,1} filtering matters."""
        vcf_path, _ = im_model_data
        bi01 = allel.GenotypeArray(
            allel.read_vcf(vcf_path)['calldata/GT']).count_alleles().is_biallelic_01()
        assert not bi01.all()
