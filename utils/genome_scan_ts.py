...
Loads genotype data from a tree sequence file (.trees or .tsz), computes windowed
and scalar statistics on the GPU, and produces a multi-panel genome scan figure.

Usage
-----
    # uncompressed tree sequence, single population
    python genome_scan_ts.py data.trees
    
    # compressed tree sequence, two populations
    python genome_scan_ts.py data.tsz
    
    # with custom windows
    python genome_scan_ts.py data.tsz --window-size 50000 --step-size 10000

The population info (sample, population) will be loaded from the metadata.
...
def parse_args():
    ...
    p.add_argument("input", help="path to tree sequence (.trees or .tsz)")
    p.add_argument("--region", help="genomic interval, e.g. '1000000-5000000'")
    ...
    p.add_argument("-o", "--output", default="genome_scan_ts.pdf",
                   help="output figure path (default: genome_scan_ts.pdf)")
    return p.parse_args()


def load_data(args):
    """Load tree sequence, optionally trim to region, apply accessibility mask, assign populations."""
    import tskit
    
    if args.input.endswith(".tsz"):
        import tszip
        ts = tszip.load(args.input)
    ...
    if args.accessible_bed:
        ...
        hm = hm.set_accessible_mask(accessible)

    if ts.num_populations > 0:
        sample_idx = {node: i for i, node in enumerate(ts.samples())}
        pop_names = {p.id: p.metadata.get("name", f"pop{p.id}") for p in ts.populations()}
        sample_sets = {}
        for ind in ts.individuals():
            if ind.population == tskit.NULL:
                continue
            pop = pop_names[ind.population]
            indices = [sample_idx[n] for n in ind.nodes if n in sample_idx]
            sample_sets.setdefault(pop, []).extend(indices)
        if sample_sets:
