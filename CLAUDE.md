...
3. **FrequencySpectrum** (`pg_gpu/diversity.py`): Power-user SFS interface for custom weight functions, SFS projection (Gutenkunst et al. 2009), and the general Achaz (2009) neutrality test framework. All theta estimators are linear combinations of the SFS; neutrality test variances use a single general formula (Achaz 2009 Eq. 9) via the Fu (1995) covariance matrix. Use `diversity.diversity_stats()` for batch computation or `FrequencySpectrum` directly for custom estimators.
...
5. **Memory Management** (`pg_gpu/_memutil.py`): GPU allele counting via fused CUDA kernel (`dac_and_n`). Adaptive chunked GPU processing for large operations.

### Key Design Patterns

- **Missing Data Handling**: Two modes only. `'include'` (default) uses per-site valid data — unbiased under MCAR. `'exclude'` drops sites with any missing. Missing values (-1) are never treated as reference alleles. LD functions accept `estimator='sigma_d2'` for unbiased projection estimators (separate from missing data handling).
- **Accessible Site Masks**: BED-based accessibility masks on HaplotypeMatrix/GenotypeMatrix. `set_accessible_mask()` is non-destructive: original data is preserved in `_haplotypes`/`_positions`, and the `haplotypes`/`positions` properties transparently return filtered views. Masks can be replaced or removed at any time. Used for per-base normalization (`span_denominator='accessible'`), gap scaling in selection scans (iHS, XPEHH). Mask stays on CPU; uses lazy prefix-sum for O(1) range queries.
...
- Missing data is encoded as -1 in haplotype matrices. Multiallelic alleles (2, 3, etc.) are folded to derived (any non-reference counts as 1 derived allele) by `chunked_dac_and_n` in `_memutil.py`.
- All functions default to `missing_data='include'` (per-site valid data)
- Two modes: 'include' (skip missing per-site, unbiased under MCAR) and 'exclude' (drop sites with any missing)
- `n_total_sites` on HaplotypeMatrix/GenotypeMatrix tracks callable site count for span normalization
