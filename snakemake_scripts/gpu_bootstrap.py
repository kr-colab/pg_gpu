...
def main():
    p = argparse.ArgumentParser("Bootstrap + plot GPU LD across all replicates")
    p.add_argument("--gpu-ld-dir", required=True, type=str)
    p.add_argument("--gpu-results-dir", required=True, type=str)
    p.add_argument("--num-reps", required=True, type=int)
    p.add_argument("--num-rbins", required=True, type=int)
    args = p.parse_args()

    ld_dir = Path(args.gpu_ld_dir)
    res_dir = Path(args.gpu_results_dir)
    ...
    print("[GPU] bootstrapping + plotting")
