...
# ---------------------------------------------------------------------------
# Fused IHH kernel for cross-population stats (xpEHH)
#
# Like ihh01_fused_kernel but computes IHH over ALL pairs (no allele
# partition). One block per focal variant, backward scan with bitmask
# pair tracking. O(n_variants) memory.
# ---------------------------------------------------------------------------
...
void ihh_fused_kernel(
    ...
    double* out_ihh           // (n_variants,)
) {
    int vi = blockIdx.x;     // focal variant
    int tid = threadIdx.x;
    int nthreads = blockDim.x;

    __shared__ int    s_ident;       // identical pairs this step
    __shared__ double s_ihh;         // accumulating IHH
    __shared__ double s_ehh_prev;
    __shared__ int    s_done;
    __shared__ int    s_n_alive_init; // initial alive pair count

    // ── classify pairs: both alleles must be non-missing at focal ─────
    int ppthread = (n_pairs + nthreads - 1) / nthreads;
    int p_start = tid * ppthread;
    int p_end   = p_start + ppthread;
    if (p_end > n_pairs) p_end = n_pairs;
    ...
    // 4 uint32 words = 128 bits per thread
    unsigned int alive[4] = {0,0,0,0};
    int local_alive_init = 0;

    for (int p = 0; p < n_local; p++) {
        int gp = p_start + p;
        long long row_off = (long long)vi * n_haplotypes;
        signed char a1 = h[row_off + pair_j[gp]];
        signed char a2 = h[row_off + pair_k[gp]];
        // Both must be non-missing; all valid pairs start alive
        if (a1 >= 0 && a2 >= 0) {
            int w = p >> 5;
            alive[w] |= 1u << (p & 31);
            local_alive_init++;
    ...
    // Count total alive pairs
    if (tid == 0) { s_n_alive_init = 0; }
    __syncthreads();
    if (local_alive_init) atomicAdd(&s_n_alive_init, local_alive_init);
    __syncthreads();

    int total_pairs = s_n_alive_init;
    if (total_pairs == 0) {
    ...
    for (int d = 1; d <= vi; d++) {
        ...
        int check_vi = vi - d;
        long long check_off = (long long)check_vi * n_haplotypes;
        int local_ident = 0;
        ...
        for (int p = 0; p < n_local; p++) {
            ...
            int gp = p_start + p;
            signed char a1 = h[check_off + pair_j[gp]];
            signed char a2 = h[check_off + pair_k[gp]];
        ...
        if (tid == 0) { s_ident = 0; }
        __syncthreads();
        if (local_ident) atomicAdd(&s_ident, local_ident);
        __syncthreads();
        ...
        if (tid == 0) {
            ...
            if (gap < 0.0) {
                ...
                double ehh = (double)s_ident / total_pairs;
                s_ihh += gap * (ehh + s_ehh_prev) * 0.5;
                s_ehh_prev = ehh;
                if (ehh <= min_ehh) s_done = 1;
            }
        }
        __syncthreads();
    }
    ...
    if (tid == 0) {
        out_ihh[vi] = (s_done || include_edges) ? s_ihh : (0.0/0.0);
    }
}
''', 'ihh_fused_kernel')
...
def _ihh01_scan_hist_gpu(h, gaps, min_ehh=0.05, min_maf=0.05,
    ...
    if max_ssl_cap is None:
        max_ssl_cap = min(n_variants, 1_000)
...
def _ihh_scan_fused_gpu(h, gaps, min_ehh=0.05, include_edges=False):
    """Forward scan computing IHH via fused kernel (one block per variant).

    Mirrors _ihh01_fused_kernel but without allele partitioning — computes
    IHH over all pairs. Each thread tracks pairs via bitmask registers.
    Supports up to ~256 haplotypes (32K pairs).
    ...
    out_ihh = cp.empty(n_variants, dtype=cp.float64)
    ...
    _ihh_fused_kernel(
        ...
        (h_contig, gaps_contig,
         np.int32(n_variants), np.int32(n_haplotypes),
         pair_j, pair_k, np.int32(n_pairs),
         np.float64(min_ehh), np.int32(int(include_edges)),
         out_ihh))
