...
output per GPU thread with all arithmetic in registers. DD uses the
ld_statistics_genotype polynomial formulas (few calls, already fast).
"""
import cupy as cp
...
# ---------------------------------------------------------------------------
# Fused CUDA kernels for genotype Dz
# ---------------------------------------------------------------------------
...
def _compute_all_dz_geno(pops, dz_calls):
    """Batch-compute all raw Dz values for genotype data using fused kernels."""
    P = len(pops)
    N = pops[0].n.shape[0]

    # Build g in moments order and precompute features
    ...
    _launch_fused(_DZ_PRECOMP_KERN,
    ...
    for case in groups:
        ...
        if case == 'diff':
            ...
            _launch_fused(_DZ_DISTINCT_KERN,
